...
def run_model(fold, hyperparams, n_epochs=100, display_epoch_info=True):  
    ...
    model2 = MLP(layers=[(FP_SIZE+(WV_SIZE*N_SOCS), 5000), (5000, 2500), (2500, N_SES)], activation="sigmoid")
    ...
    def train(loader):
        ...
        for data in loader:  # Iterate in batches over the training dataset.
            ...
            cat_input = torch.empty((len(x), FP_SIZE+(WV_SIZE*N_SOCS)), dtype=torch.float32).to(device)
            for ind, o in enumerate(x_copy):
                curr_row = o
                curr_socs = out1_copy[ind]
                for i, v in enumerate(curr_socs):
                    curr_row = torch.cat((curr_row, v*soc_embed_mat[i]))
                cat_input[ind] = curr_row
